...
# QA-Desc: a DaemonSet schedules exactly one pod per eligible node and its status current=desired=node-count (no miscount/churn, #38/#44)
# QA-Scope: cluster
# QA-Topology: full
...
pods=$($QA_SSH "curl -s '$API/api/v1/namespaces/default/pods?labelSelector=app%3D$N'" | grep -o '"uid"' | wc -l | tr -d ' ')
