...
echo "$sel" | grep -q "${P}-a" || { echo "labelSelector missed ${P}-a"; exit 1; }
echo "$sel" | grep -q "${P}-b" && { echo "labelSelector leaked ${P}-b"; exit 1; }
fs=$(curl -s "$API/api/v1/namespaces/default/configmaps?fieldSelector=metadata.name%3D${P}-a")
echo "$fs" | grep -q "${P}-a" || { echo "fieldSelector missed ${P}-a"; exit 1; }
echo "$fs" | grep -q "${P}-b" && { echo "fieldSelector leaked ${P}-b"; exit 1; }
