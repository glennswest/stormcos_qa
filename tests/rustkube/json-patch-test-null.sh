...
# QA-Desc: cilium-operator's node-taint CAS [{test /spec/taints null},{add /spec/taints [...]}] succeeds (evanphx leniency), not "path is invalid"
# QA-Scope: cluster
...
code=$(curl -s -o /dev/null -w '%{http_code}' -X PATCH -H 'Content-Type: application/json-patch+json' \
  -d '[{"op":"test","path":"/spec/taints","value":null},{"op":"add","path":"/spec/taints","value":[{"key":"node.cilium.io/agent-not-ready","effect":"NoSchedule"}]}]' \
  "$API/api/v1/nodes/$N")
[ "$code" = 200 ] || { echo "taint CAS json-patch got $code (want 200)"; exit 1; }
